...
const Appointment = () => {
            ...
            slotTypes.forEach(slotType => {
                ...
                while (slotTime < slotEnd) {
                    ...
                    const slotTypeBookings = slotsBookedForDate.filter(bookedTime => {
                        ...
                        return (bookedHour >= slotType.start && bookedHour < slotType.end)
                    })
    ...
    const bookAppointment = async () => {
        ...
        // Validate symptoms selection (now optional - warning only)
        if (selectedSymptoms.length === 0) {
            setSymptomError(true)
            toast.warning('No symptoms selected. You can still proceed.')
        } else {
        ...
        try {
            ...
            if (data && data.success) {
                ...
                const ticketData = {
                    ...
                    whatsappLink: data?.whatsappLink || null, // Include WhatsApp link if available
                    actualPatient: patientData || null // Store for reference
                }

                // If Pay on Visit, complete booking directly
                if (paymentMethod === 'payOnVisit') {
                    ...
                    // Show ticket modal after a brief delay to show success message
                    setTimeout(() => {
                        setShowTicket(true)
                        setBookingSuccess(false)
                    }, 500)
